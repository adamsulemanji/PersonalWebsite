...
import { projects, colorMap, type Project } from '@/assets/projects';
import { analyticsAttributes } from '@/lib/analytics';

function CategoryPills({ project }: { project: Project }) {
  const badgeColor = colorMap[project.color ?? 'blue'];
  return (
...
function ProjectDate({ date }: { date: string }) {
  return (
    <p className='flex items-center text-xs text-gray-500 dark:text-white/70'>
      <span className='mr-2 h-2 w-2 rounded-full bg-gray-400 dark:bg-white/50' />
      {date}
    </p>
  );
}
...
/** Stacked card for small screens: image on top, always-visible details. */
...
/** Fixed-height card for large screens; hover/focus slides up a detail panel. */
function DesktopCard({ project }: { project: Project }) {
  return (
